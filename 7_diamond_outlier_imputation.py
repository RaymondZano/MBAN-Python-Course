...
"""
help(pd.DataFrame.boxplot) #learn this 
...
diamonds.boxplot(column = ['carat'],
                  by = 'channel',) #automatically gives a title
...
plt.xlabel('Color')
#kde false - no line
#rug - know the count of each data
...
plt.axvline(x = carat_limit_2,
            linestyle = '--',
            color = 'green') #the three lines in the graph
...
diamonds['out_price'] = 0 #created a new column out_price with initial value 0


# Building a for loop
for val in enumerate(diamonds.loc[ : , 'price']): #enumerate within price
    print(val)
    
    if val[1] > price_limit_hi:
        diamonds.loc[val[0], 'out_price'] = 1 #puts 1 for prices that are outlier
#good loop - use it forever
